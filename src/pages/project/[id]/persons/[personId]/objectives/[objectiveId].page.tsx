...
export default function ObjectivePage() {
  const { loading, commentInPerson } = useContext(ProjectsContext)

  const router = useRouter()
  const { id, personId, objectiveId } = router.query
  usePreventBack(`/project/${id}/persons/${personId}`)

  const { project, permission, projectName, personsThisProject } = useProject(
    id as string,
  )
  const { person, findObjective, personName } = usePerson(personId as string)
